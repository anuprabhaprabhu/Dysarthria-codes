...
IP_dict = {'M04':.02, 'F03':.06, 'M12':.074, 'M01':.15, 'M07':.28, 'F02':.29, 'M16':.43,'M05':.58, 
        'M11':.62,'F04':.62, 'M09':.86,'M14':.904, 'M10':.93,'M08':.93,'F05':.95}
# IP_dict = {.02:'M04', .06:'F03', .074:'M12', .15:'M01', .28:'M07', .29:'F02', .43:'M16',.58:'M05', 
#         .62:'M11',.62:'F04' , .86:'M09', .904:'M14', .93:'M10',.93:'M08',.95:'F05'}
...
spr_SI = {cls: cls_value for cls, cls_value in IP_dict.items()}
print(spr_SI)
##   main 
...
for file in files:
    ...
    df1['spe_intelli'] = df1['spkrs'].map(spr_SI)
    print(spr_SI)
    ...
    with open (out_file, 'a') as f1:
        ...
        for i in ['acc','avg_proba','positive_pred']:
            pc = pearsonr(df1['spe_intelli'], df1[i] )
            sp = spearmanr(df1['spe_intelli'], df1[i] )
            rmse = root_mean_squared_error(df1['spe_intelli'], df1[i])
            # print(f'For {i} : {pc} \n')
            # print(f'For {i} : {sp} \n')
            # print(f'For {i} : {rmse} \n')
            # sys.exit()
