...
df_ctrl = pd.read_csv(ct) #, sep=',', names=['audio_path', 'text', 'label'])
df_dysar = pd.read_csv(dy)
#get unique speaker
...
for idx,spkr in enumerate(uni_spkrs_ctrl):
    # idx = 11    ####### used to prep set12
    # uni_spkrs_ctrl=np.append(uni_spkrs_ctrl,'CF03')   ####### used to prep set12

    print(idx,uni_spkrs_ctrl[idx] )
    print(idx,uni_spkrs_dysar[idx] )
    # sys.exit()
    data_fldr = os.path.join(pth, f'set{idx}')
